...
#SHOULD BE inserted seperate and Then grouped ## INVENTORY
...
sed -i 's#deny_all#htpasswd_auth provider#g' /etc/origin/master/master-config.yaml
sed -i 's#HTPasswdPasswordIdentityProvider#HTPasswdPasswordIdentityProvider \n      file: /etc/origin/master/htpasswd #g' /etc/origin/master/master-config.yaml

yum install httpd-tools -y
touch /etc/origin/master/htpasswd
htpasswd -b /etc/origin/master/htpasswd admin equiinfra
...
############################################## REPOS
[equinox-rhel-7-server-ose-3.11-rpms]
name=Equinox rhel-7-server-ose-3.11-rpms
baseurl=http://resources/rhel-repo/rhel-7-server-ose-3.11-rpms/Packages       
enabled=1
gpgcheck=0

[equinox-rhel-7-server-ansible-2.6-rpms]
name=Equinox rhel-7-server-ansible-2.6-rpms
baseurl=http://resources/rhel-repo/rhel-7-server-ansible-2.6-rpms/Packages
enabled=1
gpgcheck=0

[equinox-rhel-7-server-extras-rpms]
